...
function buildList(options) {
    ...
    tableEl.bootstrapTable({
        ...
        responseHandler: function(res) {
            var rows = res.data.list || res.data;
            if ($.isPlainObject(rows)) {
                rows = [rows];
            }
            return {
                rows: rows,
                total: res.data.totalCount || res.data.length || rows.length
            };
        },
        pagination: true,
...
function buildDetail(options) {
    ...
    for (var i = 0, len = fields.length; i < len; i++) {
        ...
        if (item.type == 'title') {
            ...
            html += '<li class="clearfix" type="' + ((item.amount || item.amount1) ? 'amount' : '') + '" style="' + (item.width ? ('width: ' + item.width + ';display:inline-block;') : '') + (item.hidden ? 'display: none;' : '') + '"><label>' + (item.help ? '<i data-help="' + item.help + '" class="zmdi zmdi-help-outline field-help"></i>' : '') + (item.title ? ('<b>' + ((item.required && '*') || '') + '</b>' + item.title + imgLabel + ':') : '&nbsp;') + '</label>';
            if (item.type == 'radio') {
                for (var k = 0, len1 = item.items.length; k < len1; k++) {
                    var rd = item.items[k];
                    ...
                    if (item.type == 'm2o') {
                        ...
                        if (item.pageCode) {
                            ...
                            if (item.searchParams) {
                                searchParams = $.extend(searchParams, item.searchParams);
                            }
                            var options1 = {
                        ...
                        if ($.isArray(realValue)) {
                            var imgsHtml = '';
                            // realValue.forEach(function(src) {
                            //     imgHtml += '<img src="' + src + '" style="max-width: 300px;"/>';
                            // });
                            var defaultFile = getDefaultFileIcon();

                            realValue.length && realValue.forEach(function(item) {
                                ...
                                if (isAcceptImg(suffix) || fields[i].arbitrarily) {
                                    imgsHtml += '<div class="img-ctn" data-src="' + src1 + '" style="display: inline-block;position: relative;">' +
                                        ...
                                        '</div>';
                                }else if (isDocOrAviOrZip(suffix)) {
...
function chosen() {
    $('select').chosen && $('select').not('.norender').chosen({
